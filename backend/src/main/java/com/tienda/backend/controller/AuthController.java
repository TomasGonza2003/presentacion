...
import com.tienda.backend.dto.request.SignupRequest;
import com.tienda.backend.dto.response.JwtResponse;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            Role userRole = roleRepository.findByName(Role.RoleName.ROLE_CLIENTE)
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin":
                        ...
                        break;
                    case "vendedor":
                        Role modRole = roleRepository.findByName(Role.RoleName.ROLE_VENDEDOR)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(modRole);
